...
const (
	...
	// CLI version is below the server's min_block threshold; server returned 426.
	CodeCLITooOld Code = "CLI_TOO_OLD"
...
func FromHTTP(statusCode int, requestID string, body []byte) *Error {
	...
	// 426 is the server's hard-block on outdated CLIs. Body shape is
	// flat JSON with code + upgrade_command + min_version (no error envelope).
	if statusCode == http.StatusUpgradeRequired {
		e.Code = CodeCLITooOld
		e.Hint = upgradeHintFromBody(body)
		return e
	}
...
// upgradeHintFromBody renders a Hint for a 426 response. Body shape (set
// by hodor's cliVersionGate):
//
//	{"error":"CLI version too old","code":"CLI_TOO_OLD",
//	 "current_version":"0.0.5","min_version":"0.1.0",
//	 "upgrade_command":"plivo upgrade"}
func upgradeHintFromBody(body []byte) string {
	var b struct {
	...
	if b.UpgradeCommand == "" {
		b.UpgradeCommand = "plivo upgrade"
	}
	if b.MinVersion == "" {
		return "Your Plivo CLI is too old. Run `" + b.UpgradeCommand + "`."
	}
	if b.CurrentVersion != "" {
		return "Your Plivo CLI (" + b.CurrentVersion + ") is older than the minimum supported " +
			b.MinVersion + ". Run `" + b.UpgradeCommand + "`."
	}
